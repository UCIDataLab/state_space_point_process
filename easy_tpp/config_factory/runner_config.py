...
from easy_tpp.config_factory.model_config import BaseConfig, ModelConfig, TrainerConfig
...
class RunnerConfig(Config):
    ...
    def parse_from_yaml_config(yaml_config, **kwargs):
        ...
        if not direct_parse:
            ...
            data_config = DataConfig.parse_from_yaml_config(
                yaml_exp_config.get("data_config")
            )
    ...
    def update_config(self):
        ...
        if self.base_config.stage == "train":
            model_folder_name = (
                f"{self.base_config.dataset_id}/{self.base_config.model_id}/{self.base_config.stage}_"
                + datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
            )
        elif self.base_config.stage == "eval":
            model_folder_name = (
                "/".join(self.model_config.pretrained_model_dir.split("/")[2:][:-1])
                + f"/{self.base_config.stage}_"
                + datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
            )
        else:
        ...
        self.model_config.num_event_types_pad = (
            self.data_config.data_specs.num_event_types_pad
        )
